...
class ADF:
                ...
                for child_name in node.children:
                    ...
                    if hasattr(self, 'facts') and child_name in self.facts:
                        ...
                        for fact_name, value in self.facts[child_name].items():
                            inherited[f"{child_name}_{fact_name}"] = value
                    else:
    ...
    def visualiseNetworkWithSubADMs(self, case=None):
        ...
        sub_adm_count = 0
        print(f"DEBUG: Looking for sub-ADMs in {len(self.nodes)} nodes...")
        print(f"DEBUG: Node names: {list(self.nodes.keys())}")
        for node_name, node in self.nodes.items():
            print(f"DEBUG: Checking node {node_name} for sub_adf_creator...")
            print(f"DEBUG: Node type: {type(node)}")
            print(f"DEBUG: Node attributes: {dir(node)}")
            print(f"DEBUG: Has sub_adf_creator: {hasattr(node, 'sub_adf_creator')}")
            if hasattr(node, 'sub_adf_creator'):
                print(f"DEBUG: Found sub-ADM creator in {node_name}")
                print(f"DEBUG: sub_adf_creator type: {type(node.sub_adf_creator)}")
                print(f"DEBUG: sub_adf_creator value: {node.sub_adf_creator}")
                sub_adm_count += 1
                try:
                    # Create sub-ADM instance
                    print(f"DEBUG: Creating sub-ADM instance for {node_name}...")
                    sub_adf = node.sub_adf_creator()
                    print(f"DEBUG: Sub-ADM created successfully: {type(sub_adf)}")
                    print(f"DEBUG: Sub-ADM name: {sub_adf.name}")
                    print(f"DEBUG: Sub-ADM nodes: {list(sub_adf.nodes.keys())}")
                    
                    # Create sub-ADM graph
                    print(f"DEBUG: Creating sub-ADM graph for {node_name}...")
                    sub_graph = sub_adf.visualiseNetwork()
                    print(f"DEBUG: Sub-ADM graph created: {type(sub_graph)}")
                    
                    # Create sub-ADM subgraph on the right
                    sub_subgraph = pydot.Subgraph(f'cluster_sub_{sub_adm_count}')
                    sub_subgraph.set_label(f'Sub-ADM: {node_name}')
                    
                    # Copy all nodes and edges from sub graph to sub subgraph
                    ...
                    print(f"DEBUG: Successfully added sub-ADM {node_name} to visualization")
                    
                    # Add connection edge from main BLF to sub-ADM
                    # Connect the current node (which has the sub-ADM) to the sub-ADM cluster
                    connection_edge = pydot.Edge(
                        node_name,  # Use the current node name instead of source_blf
                        f'cluster_sub_{sub_adm_count}',
                        style='dashed',
                        color='red',
                        label=f'→ sub-ADM'
                    )
                    combined_graph.add_edge(connection_edge)
                    print(f"DEBUG: Added connection edge from {node_name} to sub-ADM")
                    ...
                    import traceback
                    traceback.print_exc()
                    print(f"ERROR: Full error details above")
            else:
                print(f"DEBUG: Node {node_name} does not have sub_adf_creator attribute")
        
        print(f"DEBUG: Found {sub_adm_count} sub-ADMs")
        if sub_adm_count == 0:
...
class SubADMBLF(Node):
    ...
    def evaluateSubADMs(self, ui_instance):
        ...
        """
        try:
            print(f"\nEvaluating sub-ADMs for {self.name}...")
            
            # Get the list of items from the source BLF
            if not hasattr(ui_instance.adf, 'facts') or self.source_blf not in ui_instance.adf.facts:
                print(f"Warning: No items found from {self.source_blf}")
                return False
            
            # Get the items list (assuming it's stored as a fact)
            items = ui_instance.adf.facts[self.source_blf].get('items', [])
            
            # If no items found, try to get them from the source BLF's facts
            if not items:
                print(f"No items found in {self.source_blf}.facts['items'], checking if source BLF needs to be evaluated first...")
                # The source BLF should have already been evaluated and stored its items
                # This is handled by the domain configuration, not hardcoded here
                return False
            
            print(f"Found {len(items)} items to evaluate: {items}")
            
            positive_count = 0
            negative_count = 0
            item_results = []  # Store detailed results for each item
            ...
            for item in items:
                ...
                try:
                    # Run the sub-ADM evaluation
                    sub_result, sub_case = self._evaluateSubADM(sub_adf, item)
                    self.sub_adf_results[item] = sub_result
                    item_results.append(sub_case)  # Store the full case, not just the result
                    
                    if sub_result == 'POSITIVE':
                        positive_count += 1
                        print(f"Result: {item} is POSITIVE")
                    elif sub_result == 'NEGATIVE':
                        negative_count += 1
                        print(f"Result: {item} is NEGATIVE")
                    else:
                        print(f"Result: {item} is UNKNOWN")
                        pass
            ...
            if hasattr(ui_instance.adf, 'setFact'):
                ...
                print(f"DEBUG: Stored sub-ADM results: {item_results}")
                print(f"DEBUG: Positive count: {positive_count}, Negative count: {negative_count}")
            
            # Determine final acceptance based on results
            print(f"\n--- Final Results ---")
            print(f"Positive items: {positive_count}")
            print(f"Negative items: {negative_count}")
            ...
            # print(f"Error evaluating sub-ADMs: {e}")
            return False
