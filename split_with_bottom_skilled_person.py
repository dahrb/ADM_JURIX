#!/usr/bin/env python3
"""
Split ADM Visualization with SkilledPerson at Bottom
Creates separate images with SkilledPerson positioned at the bottom of Sub-ADM 2
"""
...
def create_sub_adm_2_with_bottom_skilled_person(adf, sub_adf, sub_adm_num):
    """
    Create Sub-ADM 2 visualization with SkilledPerson positioned at the bottom
    """
    # Create a copy of the sub-ADM and add SkilledPerson as a proper node
    enhanced_sub_adf = SubADM(f"Sub-Model {sub_adm_num}", "enhanced")
    
    # Copy all existing nodes from the original sub-ADM
    for node_name, node in sub_adf.nodes.items():
        enhanced_sub_adf.nodes[node_name] = node
    
    # Copy other attributes
    enhanced_sub_adf.questionOrder = sub_adf.questionOrder.copy()
    enhanced_sub_adf.case = sub_adf.case.copy()
    
    # Add SkilledPerson as a proper node using addNodes (as a leaf node)
    enhanced_sub_adf.addNodes("SkilledPerson", 
                             question="Is the practitioner skilled in the relevant technical field?")
    
    # Add NonTechnicalContribution with red styling
    enhanced_sub_adf.addNodes("NonTechnicalContribution", 
                             question="Is there a non-technical contribution?")
    
    # Create the visualization using the enhanced sub-ADM
    sub_graph = enhanced_sub_adf.visualiseNetwork()
    ...
    # Add all nodes and edges from the enhanced sub-ADM to the subgraph
    for sub_node in sub_graph.get_node_list():
    ...
    if "Hindsight" in enhanced_sub_adf.nodes:
    ...
    # Force SkilledPerson to be at the bottom by creating a rank constraint
    # Create a rank subgraph for the bottom level
    bottom_rank = pydot.Subgraph(rank='same')
    bottom_rank.add_node(pydot.Node("SkilledPerson"))
    combined_graph.add_subgraph(bottom_rank)
    
    combined_graph.add_subgraph(sub_subgraph)
    
    return combined_graph

def create_inventive_step_split_with_bottom_skilled_person():
    """
    Create split visualizations for Inventive Step ADM with SkilledPerson at bottom
    """
    print("=" * 60)
    print("INVENTIVE STEP ADM - SPLIT WITH BOTTOM SKILLEDPERSON")
    print("=" * 60)
        ...
        if hasattr(node, 'sub_adf_creator'):
            ...
            if sub_adm_key not in sub_adm_mapping:
                ...
                try:
                    ...
                    if sub_adm_count == 2:
                        sub_graph = create_sub_adm_2_with_bottom_skilled_person(adf, sub_adf, sub_adm_count)
                        print(f"✓ Created Sub-ADM {sub_adm_count} with SkilledPerson at bottom")
                    else:
    ...
    print(f"  • Sub-ADM 2 includes SkilledPerson at bottom with red outer ring")
...
def main():
    """
    Generate split visualizations for Inventive Step ADM with SkilledPerson at bottom
    """
    print("Creating split Inventive Step ADM visualizations...")
    print("Sub-ADM 2 will position SkilledPerson at the bottom")
    
    try:
        sub_adm_count = create_inventive_step_split_with_bottom_skilled_person()
        ...
        print("\nSub-ADM 2 includes:")
        print("• SkilledPerson as proper ADM node with red outer ring")
        print("• SkilledPerson positioned at the bottom")
        ...
        print("• inventive_step_sub_adm_2.png/svg (with SkilledPerson at bottom)")
